    ...
    docker.image('python-dind').inside('--privileged --user root') {
        stage('Build') {
            try {
                ...
                sh 'python -m py_compile sources/add2vals.py sources/calc.py > log.txt 2>&1 && cat log.txt'
        ...
        stage('Test') {
            try {
                ...
                sh 'pytest --verbose --junit-xml=test-reports/results.xml sources/test_calc.py > log.txt 2>&1 && cat log.txt'
        ...
        stage('Create Executable') {
            try {
                ...
                sh 'pyinstaller --onefile sources/add2vals.py > log.txt 2>&1 && cat log.txt'
        ...
        stage('Deploy') {
            try {
                withCredentials([sshUserPrivateKey(credentialsId: '8b0b5e66-4b7d-4887-953c-6b114a06cb90', keyFileVariable: 'AWS_SSH_KEY', usernameVariable: 'SSH_USER')]) {
                    ...
                    sh 'scp -i $AWS_SSH_KEY -r ./dist/* $SSH_USER@54.169.12.75:/home/ubuntu/my-python-app/ > log.txt 2>&1 && cat log.txt'
